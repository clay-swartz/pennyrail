...
async function boundedText(response: Response) {
  ...
  if (!reader) {
    ...
    if (Buffer.byteLength(text, "utf8") > MAX_BYTES) throw new Error("response too large");
    return text;
  }
...
function titleFromHtml(html: string) {
  ...
  return decodeEntities(raw.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim()) || null;
}
...
async function extractOne(rawUrl: string, includeText: boolean, includeHighlights: boolean) {
...
async function handler(req: NextRequest) {
  try {
    ...
    const urls = rawUrls.map((v: unknown) => String(v || "").trim()).filter(Boolean);
    if (!urls.length) return NextResponse.json({ error: "urls is required" }, { status: 400 });
    if (urls.length > MAX_URLS) {
      return NextResponse.json({ error: `maximum ${MAX_URLS} URLs per call` }, { status: 400 });
    }

    if (body?.summary) {
      return NextResponse.json({
        error: "summary mode is not offered on the $0.001 tier; request text/highlights instead",
      }, { status: 400 });
    }
