...
const DEMAND_SNIPERS = [
  { path:"/api/agent/random", priceUsd:0.0005, intents:["random","secure random","random bytes","random integer","cryptographic randomness"], description:"Cryptographically secure randomness for repeated agent workflows." },
  { path:"/api/agent/uuid", priceUsd:0.0005, intents:["uuid","uuid generator","uuid v4","uuid v7","generate uuid"], description:"UUID v4/v7 generation for repeated machine workflows." },
  { path:"/api/agent/hash", priceUsd:0.0005, intents:["hash","hashing","sha256","sha512","sha-256","sha-512","cryptographic hash","digest","checksum"], description:"SHA-256/SHA-512/SHA-1/MD5 hashing with hex and Base64 digests." },
  { path:"/api/agent/base64-decode", priceUsd:0.0005, intents:["base64","base64 decode","decode base64","encoding"], description:"Base64 decoding to UTF-8." },
  { path:"/api/agent/hex-decode", priceUsd:0.0005, intents:["hex","hex decode","decode hex","encoding"], description:"Hexadecimal decoding to UTF-8." },
  { path:"/api/agent/token-count", priceUsd:0.0005, intents:["token count","count tokens","llm tokens","bpe tokens","context budget"], description:"Exact OpenAI BPE token counting." },
  { path:"/api/agent/time-convert", priceUsd:0.0005, intents:["time convert","epoch to iso","iso to epoch","timezone convert","current time"], description:"Epoch/ISO/timezone conversion." },
  { path:"/api/agent/text-stats", priceUsd:0.0005, intents:["text stats","word count","character count","line count"], description:"Deterministic text statistics." },
  { path:"/api/agent/dns", priceUsd:0.0005, intents:["dns","dns lookup","mx lookup","txt record","cname","a record"], description:"DNS record lookup via DNS-over-HTTPS." },
  { path:"/api/agent/block-number", priceUsd:0.0005, intents:["block number","latest block","evm block","onchain read","crypto data"], description:"Latest EVM block number." },
  { path:"/api/agent/weather", priceUsd:0.0015, intents:["weather","current weather","weather now","weather by city"], description:"Current global weather." },
  ...
  { path:"/api/agent/page-metadata", priceUsd:0.0015, intents:["page metadata","article metadata","open graph","website metadata","url metadata"], description:"Page/article metadata extraction." },
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    ...
    version: 41,
    name: "PennyRail",
    summary: "Autonomous x402 demand sniper: watches paid and unmet agent demand, auto-publishes matching capabilities, and sells high-frequency machine work below common market prices.",
    homepage: origin,
    resources: [
    ...
    routing: {
      featured: DEMAND_SNIPERS.map(item => ({
