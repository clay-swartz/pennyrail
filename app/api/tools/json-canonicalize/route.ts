...
const handler=async(req:NextRequest)=>{try{const input=await req.json();const value='value'in input?input.value:input;const canonical=JSON.stringify(sort(value));return NextResponse.json({canonical,bytes:new TextEncoder().encode(canonical).length})}catch{return NextResponse.json({error:'invalid JSON body'},{status:400})}};
